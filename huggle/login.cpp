...
void Login::PerformLoginPart2(WikiSite *site)
{
    ...
    {
        this->CancelLogin();
        this->Update(_l("login-fail", site->Name) + ": " + _l("no-valid-login-token-returned"));
        return;
    }
...
void Login::RetrieveUserConfig(WikiSite *site)
{
    if (this->LoginQueries.contains(site))
    {
        ...
        if (q->IsProcessed())
        {
            if (q->IsFailed())
            {
                this->DisplayError(_l("login-error-retrieve-config", q->Result->ErrorMessage));
                return;
            }
...
void Login::OnTimerTick()
{
    ...
    foreach (WikiSite *site, hcfg->Projects)
    {
        if (!this->Statuses.contains(site))
            throw new Huggle::Exception(_l("no-wiki-statuses-list"), BOOST_CURRENT_FUNCTION);
        if (!this->GlobalConfig)
        {
